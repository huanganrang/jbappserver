...
<c:if test="${fn:contains(sessionInfo.resourceList, '/appResourceController/editPage')}">
...
<c:if test="${fn:contains(sessionInfo.resourceList, '/appResourceController/editHtml5Page')}">
	<script type="text/javascript">
		$.canHtml5 = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.resourceList, '/appResourceController/editMapPage')}">
	<script type="text/javascript">
		$.canMap = true;
	</script>	
</c:if>
<script type="text/javascript">
	...
	$(function() {
		treeGrid = $('#treeGrid').treegrid({
			url : '${pageContext.request.contextPath}/appResourceController/treeGrid',
			idField : 'id',
			treeField : 'name',
			parentField : 'pid',
			fit : true,
			...
			columns : [ [ {
				...
				formatter : function(value, row, index) {
					...
					if(row.typeId=="3"){
						var str = '';
						if ($.canHtml5) {
							str += $.formatString('<img onclick="editFunHtml5(\'{0}\');" src="{1}" title="设置html5"/>', row.id, '${pageContext.request.contextPath}/style/images/extjs_icons/html.png');
						}
						if ($.canMap) {
							str += $.formatString('<img onclick="editFunMap(\'{0}\');" src="{1}" title="地图设置"/>', row.id, '${pageContext.request.contextPath}/style/images/extjs_icons/mouse.png');
						}
					}else{
						var str = '';
						if ($.canEdit) {
	...
	function editFun(id) {
		...
		if (node) {
			parent.$.modalDialog({
				...
				width : 500,
				height : 200,
				href : '${pageContext.request.contextPath}/appResourceController/editPage?id=' + node.id,
				buttons : [ {
					text : '编辑',
					handler : function() {
	...
	function editFunHtml5(id) {
		...
		if (node) {
			parent.$.modalDialog({
				...
				width : 500,
				height : 200,
				href : '${pageContext.request.contextPath}/appResourceController/editHtml5Page?id=' + node.id,
				buttons : [ {
					text : '编辑',
					handler : function() {
						parent.$.modalDialog.openner_treeGrid = treeGrid;//因为添加成功之后，需要刷新这个treeGrid，所以先预定义好
